...
private_dirty = [waypoints_dirty, motor_speed_dirty, state_coordinate_dirty, state_color_dirty, obstacles_dirty, targets_dirty]

# in from: list of tuples representing objects, etc.
def vision(obstacles_lock, obstacles_dirty, obstacles, targets_lock, targets_dirty, targets):
    pass
# out to: motor speed, sensor data
# in from: our position
def localization(motor_speed_lock, motor_speed, state_coordinate_lock, state_coordinate_dirty, state_coordinate, state_color_lock, state_color_dirty, state_color):
    pass
# out to: locations, objects we see, bases, obstacles (tuples)
# in from: list of waypoints
def pathfinding(obstacles_lock, obstacles, targets_lock, targets, waypoints_lock, waypoints_dirty, waypoints):
    pass
# out to: list of waypoints
# in from: motor speed
def motor_control(waypoints_lock, waypoints, motor_speed_lock, motor_speed_dirty, motor_speed):
    # while(True):
    #     if (new_waypoints):
    #         pass
    pass
...
if __name__ == '__main__':
    ...
    vision_proc = Thread(target=vision, args=((obstacles_private_lock),(obstacles_dirty),(private_obstacles),(targets_private_lock),(targets_dirty),(private_targets),))
    localization_proc = Thread(target=localization, args=((motor_speed_public_lock),(public_motor_speed),(state_coordinate_private_lock),(state_coordinate_dirty), \
        (private_state_coordinate),(state_color_private_lock),(state_color_dirty),(private_state_color),))
    pathfinding_proc = Thread(target=pathfinding, args=((obstacles_public_lock),(public_obstacles),(targets_public_lock),(public_targets),(waypoints_private_lock), \
        (waypoints_dirty),(private_waypoints),))
    motor_control_proc = Thread(target=motor_control, args=((waypoints_public_lock),(public_waypoints),(motor_speed_private_lock),(motor_speed_dirty),(private_motor_speed),))
    ...
    num_data_vectors = range(len(names_map))
